...
static void DrawChoicesForItem(u8 taskId, u8 itemIndex, u8 y);
static void DrawVisibleOptionChoices(u8 taskId);
...
static void Task_OptionMenuProcessInput(u8 taskId)
{
    ...
    {
        u8 previousOption;
        u8 y = (gTasks[taskId].tMenuSelection - gTasks[taskId].tScrollOffset) * 16;
        ...
        case MENUITEM_TURBOA:
            previousOption = gTasks[taskId].tTurboA;
            gTasks[taskId].tTurboA = TurboA_ProcessInput(gTasks[taskId].tTurboA);
...
static void UpdateOptionMenuScroll(u8 taskId)
{
    u8 prevScrollOffset = gTasks[taskId].tScrollOffset;

    if (gTasks[taskId].tMenuSelection < gTasks[taskId].tScrollOffset)
        gTasks[taskId].tScrollOffset = gTasks[taskId].tMenuSelection;
    else if (gTasks[taskId].tMenuSelection > gTasks[taskId].tScrollOffset + MAX_VISIBLE_MENU_ITEMS - 1)
        gTasks[taskId].tScrollOffset = gTasks[taskId].tMenuSelection - (MAX_VISIBLE_MENU_ITEMS - 1);

    if (gTasks[taskId].tScrollOffset != prevScrollOffset)
    {
        DrawOptionMenuTexts(gTasks[taskId].tScrollOffset);
        DrawVisibleOptionChoices(taskId);
        CopyWindowToVram(WIN_OPTIONS, COPYWIN_GFX);
    }
...
{
    u8 i;
    u8 scrollOffset = gTasks[taskId].tScrollOffset;

    for (i = 0; i < MAX_VISIBLE_MENU_ITEMS; i++)
        DrawChoicesForItem(taskId, scrollOffset + i, i * 16);
}
...
static void HighlightOptionMenuItem(u8 visibleRow)
{
    ...
    SetGpuReg(REG_OFFSET_WIN0V, WIN_RANGE(visibleRow * 16 + 40, visibleRow * 16 + 56));
}
